...
/// });
#[macro_export]
macro_rules! device_struct {
    ($board_name:ident, { $($device_name:ident: $driver_type:ty = $value:expr),* $(,)? }) => {
        struct $board_name;
        
        crate::singleton!($board_name {});

        impl $board_name {
            $(
                #[inline]
                pub fn $device_name() -> &'static mut $driver_type {
                    static mut DEVICE: $driver_type = $value;
                    unsafe {
                        core::ptr::addr_of_mut!(DEVICE).as_mut().unwrap()
                    }
                }
