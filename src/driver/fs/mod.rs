use crate::driver::fs::littlefs::LittleFs;
...
impl Fs {
    ...
    fn path_to_fs(path: &str) -> Result<(&mut Box<dyn FsHandle>, &str)> {
        if path == "/" {
        ...
        for entry in Fs::get_mut().fstab.iter_mut() {
            if path.starts_with(entry.mount_point) {
                ...
                return Ok((&mut entry.fs, path));
            }
        }
        Err(anyhow!("Filesystem not found for path: {}", path))
    }
    fn path_to_fs_index(path: &str) -> Result<(&mut Box<dyn FsHandle>, usize, &str)> {
        if path == "/" {
            return Err(anyhow!("Root path does not belong to any filesystem"));
        }
        for (index, entry) in Fs::get_mut().fstab.iter_mut().enumerate() {
            if path.starts_with(entry.mount_point) {
                let path = &path[entry.mount_point.len()..];
                let path = if path.is_empty() { "/" } else { path };
                return Ok((&mut entry.fs, index, path));
            }
        }
        Err(anyhow!("Filesystem not found for path: {}", path))
    }
    fn mount_point_to_fs(mount_point: &str) -> Result<&mut Box<dyn FsHandle>> {
        for entry in Fs::get_mut().fstab.iter_mut() {
            if entry.mount_point == mount_point {
                return Ok(&mut entry.fs);
            }
        }
